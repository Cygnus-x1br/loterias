...
new #[Layout('layouts.app', ['title' => 'Otimização de Fechamentos'])] class extends Component
{
    use WithPagination;
    ...
    public function statusLabel(?string $status): string
    {
        return match ($status) {
            'draft' => 'Rascunho',
            'processing' => 'Em processingamento',
            'completed' => 'Concluído',
            'placed' => 'Apostado',
            'checked' => 'Conferido',
            'failed' => 'Falhou',
    ...
    <section class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
            ...
            <div class="inline-flex items-center gap-2 rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">
                ...
                Refinamento de apostas
            </div>

            <h1 class="mt-3 text-3xl font-extrabold tracking-tight text-slate-900">
                Otimizar Fechamentos
            </h1>

            <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500 sm:text-base">
                Selecione um fechamento gerado abaixo para aplicar filtros matemáticos e descartar apostas fracas.
            </p>
        </div>
    </section>
                ...
                @foreach ($closings as $closing)
                    ...
                    >
                        ...
                        <div class="flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
                            <a
                                href="{{ route('closings.optimize', $closing) }}"
                                class="inline-flex items-center justify-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700 transition hover:bg-indigo-100"
                            >
                                <svg
                                    class="h-4 w-4"
                                    fill="none"
