...
new #[Layout('layouts.app', ['title' => 'Detalhes do fechamento'])] class extends Component
{
    ...
    public ?array $combinatorialCoverage = null;
    ...
    public function checkBaseNumbersCoverage(): void
    {
        ...
        // Auditoria Combinatória: Aplicável a QUALQUER método, desde que haja apostas e uma base razoável (<= 23) para não travar a UI
        if (count($bets) > 0 && count($baseNumbers) >= 15 && count($baseNumbers) <= 23) {
            // Tenta resgatar as garantias dos parâmetros, ou assume o padrão Lotofácil (Garante 14 se acertar 15)
            $guaranteeHits = $this->closing->parameters['reduced_parameters']['guarantee_hits'] 
                ?? $this->closing->parameters['guarantee_hits'] 
                ?? 15;
                
            $guaranteePoints = $this->closing->parameters['reduced_parameters']['guarantee_points'] 
                ?? $this->closing->parameters['guarantee_points'] 
                ?? 14;

            // Extrai as apostas geradas num array limpo
            $parsedBets = [];
            foreach ($bets as $bet) {
                $parsedBets[] = is_array($bet->numbers) ? $bet->numbers : (json_decode((string) $bet->numbers, true) ?? []);
            }

            try {
                $auditor = app(\App\Services\Betting\CoverageAuditorService::class);
                $this->combinatorialCoverage = $auditor->auditCoverage(
                ...
                // Se não for o método reduzido, mudamos o texto da condição para ficar mais claro
                if ($this->closing->method !== 'reduced') {
                     $this->combinatorialCoverage['condicoes_da_garantia'] = "Análise passiva: Qual a chance matemática de fazer {$guaranteePoints} pontos acertando {$guaranteeHits} dezenas na base escolhida?";
                }
            } catch (\Exception $e) {
                $this->combinatorialCoverage = null;
            }
        }
    }
...
<div class="mx-auto max-w-7xl space-y-6">
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        @php
                            ...
                            $betPrice = app(\App\Services\LotofacilSettingService::class)->getPriceFor($closing->bet_size);
                            $totalCost = $totalBets * $betPrice;
                        @endphp
                        
                        @if($totalBets > 0)
                            <div class="mt-4 flex flex-col gap-0.5 rounded-xl bg-slate-50 border border-slate-200 p-3 max-w-sm">
                                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Estimativa de Custo</span>
                                <span class="text-lg font-black text-slate-800">R$ {{ number_format($totalCost, 2, ',', '.') }}</span>
                                <span class="text-xs text-slate-500">{{ $totalBets }} aposta{{ $totalBets > 1 ? 's' : '' }} de {{ $closing->bet_size }} dezenas (R$ {{ number_format($betPrice, 2, ',', '.') }} cada)</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2.5">
                        ...
                        @if ($closing->bets()->count() > 0)
                            ...
                            <a
                                ...
                                class="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 hover:text-indigo-600"
                            >
        ...
        @if ($combinatorialCoverage)
            <aside class="lg:col-span-1 space-y-6 mt-6 lg:mt-0">
                <dl class="rounded-2xl border border-indigo-200 bg-indigo-50/50 p-5 shadow-sm sm:p-6 space-y-4">
                    <div class="flex items-center gap-2 border-b border-indigo-100 pb-3">
                        ...
                        <h3 class="text-sm font-bold text-indigo-900">Auditoria Combinatória</h3>
                    </div>

                    <div class="text-xs text-indigo-800 font-medium pb-2 border-b border-indigo-100">
                        {{ $combinatorialCoverage['condicoes_da_garantia'] }}
                    </div>

                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-xs text-slate-500">Cenários Possíveis</dt>
                        <dd class="text-sm font-bold text-slate-800">{{ number_format($combinatorialCoverage['quantidade_cenarios_analisados'], 0, ',', '.') }}</dd>
                    </div>

                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-xs text-slate-500">Garantia Atingida?</dt>
                        <dd>
                            @if($combinatorialCoverage['garantia_confirmada'])
                                <span class="inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-700">Sim (100%)</span>
                            @else
                                <span class="inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">Parcial</span>
                            @endif
                        </dd>
                    </div>

                    <div class="space-y-2 pt-2 border-t border-indigo-100">
                        <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Cobertura por Faixa</h4>
                        @foreach([15, 14, 13, 12, 11] as $hits)
                            @php
                                $cov = $combinatorialCoverage['cobertura_por_faixa']["{$hits}_acertos"] ?? 0;
                            @endphp
                            <div class="flex items-center justify-between gap-4">
                                <dt class="text-xs text-slate-600">{{ $hits }} Acertos</dt>
                                <dd class="text-xs font-bold {{ $cov == 100 ? 'text-emerald-600' : ($cov > 0 ? 'text-indigo-600' : 'text-slate-400') }}">
                                    {{ number_format($cov, 2, ',', '') }}%
                                </dd>
                            </div>
                            <div class="w-full bg-indigo-200/50 rounded-full h-1.5">
                                <div class="bg-indigo-500 h-1.5 rounded-full" style="width: {{ $cov }}%"></div>
                            </div>
                        @endforeach
                    </div>
                </dl>
