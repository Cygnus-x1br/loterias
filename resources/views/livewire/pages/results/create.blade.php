...
new #[Layout('layouts.app', ['title' => 'Cadastrar Concurso'])] class extends Component
{
    ...
    {
        $latestContest = HistoricalResult::max('contest_number');
        if ($latestContest) {
            $this->contest_number = $latestContest + 1;
        }

        $this->draw_date = now()->format('Y-m-d');
    }
    ...
    public function rules(): array
    {
        return [
            'contest_number' => [
                ...
                'integer',
                'min:1',
                'unique:historical_results,contest_number',
            ],
            'draw_date' => [
                'required',
                'date',
    ...
    public function messages(): array
    {
        return [
            ...
            'contest_number.unique' => 'Este número de concurso já está cadastrado no sistema.',
    ...
    public function save(HistoricalResultService $service): void
    {
        ...
        try {
            ...
            session()->flash('success', "Concurso {$validated['contest_number']} cadastrado com sucesso!");
    ...
    <section class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
            ...
            <h1 class="mt-3 text-3xl font-extrabold tracking-tight text-slate-900">
                Cadastrar Sorteio da Lotofácil
            </h1>

            <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500 sm:text-base">
                Preencha os dados do concurso e selecione as 15 dezenas sorteadas.
            </p>
        </div>
            ...
            <div class="mt-6 flex flex-col gap-3 rounded-2xl border border-sky-100 bg-sky-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-start gap-3">
                    ...
                    <p class="text-sm leading-6 text-sky-800">
                        Você também pode preencher rapidamente com números aleatórios para fins de teste.
                    </p>
                </div>
                ...
                <button
                    ...
                    wire:loading.attr="disabled"
                    class="shrink-0 rounded-xl bg-sky-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                    Preencher aleatório
                </button>
            </div>
